...
echo "quantiles skipped"
python save_examples.py --model_name=$MODEL_NAME --output_dir=$OUTPUT_DIR --num_examples=$NUM_EXAMPLES
echo "saved examples"
python perplex.py --model_name=$MODEL_NAME --output_dir=$OUTPUT_DIR --prompt="$PROMPT" --high_pp_target="$HIGH_PP" --low_pp_targe\
t="$LOW_PP" --shot=$SHOT --num_examples=$NUM_EXAMPLES
echo "donso"